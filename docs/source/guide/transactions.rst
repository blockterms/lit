...
Keys in Lit allow 2 ways of handling transactions: a
:func:`~lit.PrivateKey.create_transaction` method that creates a signed
transaction and returns the aforementioned transaction in hex, and a
:func:`~lit.PrivateKey.send` method that does the same thing but will
...
By default Lit will send any change to the same address you sent from. You
...
By default, Lit will poll `<https://bitcoinfees.earn.com>`_ and use a fee that
...
By default Lit will use all of your available UTXOs to make future transactions
...
Each item must be an instance of :class:`~lit.network.meta.Unspent`.
